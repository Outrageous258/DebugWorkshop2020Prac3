    ...
    public static Result Calculate(string input)
    {
      ...
      if (operations.Length >= numbersToBeCalculated.Length)
      {
        return new Result(0, "Wrong entry. Try again using one or more operations");
            ...
            //Order operators array by / * + - then do that for the numbers array
            int index = 0;
            char[] tmpArr = new char[operations.Length];
            int[] pos = new int[numbersToBeCalculated.Length];
            int[] tmpNum = new int[numbersToBeCalculated.Length];
            for (int i = 0; i < operations.Length; i++)
            {
                if(operations[i] == '/')
                {
                    tmpArr[index] = operations[i];
                    pos[index] = i;
                    index++;
                }
            }
            ...
            {
                operations[i] = tmpArr[i];
            }
            for (int i = 0; i < numbersToBeCalculated.Length; i++)
            {
                tmpNum[i] = numbersToBeCalculated[pos[i]];
            }
